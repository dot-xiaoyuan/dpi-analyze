...
func Setup() error {
	Manager = manager.NewManager(manager.Config{
		Filename:              fmt.Sprintf("%s/brands.yaml", config.EtcDir),
		CollectionName:        types.MongoCollectionFeatureBrands, // 对应 Mongo 集合名
		HistoryCollectionName: types.MongoCollectionFeatureBrandsHistory,
